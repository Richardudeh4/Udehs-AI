  ...
  return (
    <main>
        ...
        <section>
          ...
          All AI powered sales assistant chatbot
          </span>
          <Image 
        ...
        <p className="text-muted-foreground text-center max-w-lg"> Our straightforward pricing plans are tailored to meet your need . If {"you're"} not ready to commit you can get started for free.</p>
        </section>
         ...
         pricingCards.map((card) => (
          <Card key={card.title} className={clsx("w-[300px] flex flex-col justify-between", {'border-2 border-primary': card.title === "Unlimited", })}>
            ...
            <CardFooter className="flex flex-col items-start gap-4 ">
              ...
              <Link href={`/dashboard?plan=${card.title}`} className="bg-[#f3d299] border-orange border-2 p-2 w-full text-center font-bold rounded-md">
                Get started
              </Link>
            </CardFooter>
          </Card>
        ...
        <p className="text-muted-foreground text-center max-w-lg">Explore our insight on AI, technology and optimizing your business </p>
        </section>

        <section className="md:grid-cols-3 grid-cols-1 grid gap-5 container mt-12 ">
          {
            posts && posts.map((post) =>(
              <Link href={`/blogs/${post.id}`} key={post.id}>
                <Card className="flex flex-col gap-2 rounded-xl overflow-hidden h-full hover:bg-gray-100">
                  ...
                  <Image
                  src={`${process.env.CLOUDWAYS_UPLOAD_URL}${post.image}`}
